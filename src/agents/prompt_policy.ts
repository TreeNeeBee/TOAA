...
8. Completion evidence: done=true requires all declared outputs plus successful mutation/verification evidence appropriate to this Step. In right-side V-model verification Steps, the first round may only inspect current local artifacts and run the declared executable gate. Do not diagnose from historical failure text or write reports, documentation, source, or tests until the current run_tests result is returned in a later round. A Bug requires concrete failed run_tests evidence; completeness or coverage shortfalls belong in qualityAssessment.gaps and become Enhancements.${declarative
...
8. 完成证据：done=true 前必须完成全部声明产物，并取得与当前 Step 匹配的成功修改或验证证据。V 模型右侧验证 Step 第一轮只能检查当前本地工件并运行声明的可执行门禁；在下一轮收到当前 run_tests 结果前，禁止依据历史失败文本诊断，也禁止写入报告、文档、源码或测试。只有具体的 run_tests 失败证据可以产生 Bug，完整度或覆盖率不足必须写入 qualityAssessment.gaps 并转为 Enhancement。${declarative
