...
5. Network APIs: after a failed endpoint, perform at most two focused probes, reject empty/unusable 2xx responses, patch the selected integration, then verify the deterministic Step tests. Live user-entrypoint verification belongs to the Phase delivery gate.
...
5. 网络 API：接口失败后最多进行两次有目标的探测；HTTP 2xx 但内容为空或不可用不算成功；选定接口后必须修改真实集成，并验证当前 Step 的确定性测试；真实用户入口验证只属于 Phase 交付门禁。
